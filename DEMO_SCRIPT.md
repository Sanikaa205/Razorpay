...
| 2 | 0:15–1:00 | **Onboarding** | Go to `/signup`. Sign up as a new merchant (any name/email/password). You land on the Onboarding screen. Type any string (e.g. `acct_test_demo`) into the Razorpay field and click **Connect**. Upload `demo/demo-products.csv` (20 products) via the CSV card. | "Three steps, one screen: create the account, connect Razorpay, upload the catalog. That checklist just went from 1/3 to 3/3." | All three onboarding steps show a green check; catalog shows "20 products". |
